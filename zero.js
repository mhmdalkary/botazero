...
try {
  global.client.configPath = join(global.client.mainPath, "Zeroconfig.json");
  configValue = require(global.client.configPath);
  logger.loader("Found Zeroconfig.json file!");
} catch (e) {
  return logger.loader('"Zeroconfig.json" file not found."', "error");
}
...
try {
  var appStateFile = resolve(join(global.client.mainPath, config.APPSTATEPATH || "Zerostat.json"));
  ...
  logger.loader("Found the bot's Zerostat.")
} catch (e) {
  logger.loader("Can't find the bot's Zerostat.", "error");
...
function onBot() {
  ...
  login(loginData, async (err, api) => {
    ...
    api.setOptions(global.config.FCAOption);
      fs.writeFileSync('Zerostat.json', JSON.stringify(api.getAppState()));
    ...
    global.config.version = config.version,
      (async () => {
        const commandsPath = `${global.client.mainPath}/ZeroFil/commands`;
        ...
        for (const command of listCommand) {
          try {
            ...
            if (envConfig) {
              ...
              var configPath = require('./Zeroconfig.json');
      ...
      (async () => {
        const events = readdirSync(join(global.client.mainPath, 'ZeroFil/events')).filter(ev => ev.endsWith('.js') && !global.config.eventDisabled.includes(ev));
        console.log(cv(`\n` + `──LOADING EVENTS─●`));
        for (const ev of events) {
          try {
            const event = require(join(global.client.mainPath, 'ZeroFil/events', ev));
...
This file was modified by me (@benzo. Do not steal my credits. (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
* */