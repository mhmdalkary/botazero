...
const ADMIN = 'benzo naw';
const FB_LINK = 'https://www.facebook.com/gemar.pro.52';
...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.imgur.com/FAqcy8a.gif';
let gifPath = __dirname + '/cache/join/join.gif';

axios.get(gifUrl, { responseType: 'arraybuffer' })
.then(response => {
    fs.writeFileSync(gifPath, response.data);
    return api.sendMessage("هي هناك!", event.threadID, () => api.sendMessage({ body: `✅ اتصال المجموعة في ${threadName} at ${session} نجاح! \n\n➭ الأوامر الحالية: ${commands.size}\n➭ بادئة بوت: ${global.config.PREFIX}\n➭ إصدار: ${global.config.version}\n➭ ادمن: ‹${ADMIN}›\n➭ فيسبوك: ‹ ${FB_LINK} ›\n➭ يستخدم ${PRFX}اوامر لعرض تفاصيل الأمر\n➭ تمت إضافة بوت في: ⟨ ${time} ⟩〈 ${thu} 〉`, attachment: fs.createReadStream(gifPath)}, threadID));
})
.catch(error => {
    console.error(error);
});
    ...
    try {
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        ...
        ctx.save();
        ctx.font = `75px Semi`;
        ctx.fillText(`مرحبا بك في ${threadName}`, canvas.width / 2 - 15, canvas.height / 2 + 235)
      ...
      (typeof threadData.customJoin == "undefined") ? msg = `🌟 مرحبا بالعضو الجديد {name} إلى المجموعة {threadName}\n→ URL حسابnhttps://www.facebook.com/profile.php?id={iduser}\n→ {type} هي المجموعة {soThanhVien}${suffix} عضو\n→ تمت الإضافة إلى المجموعة بواسطة: {author}\n→ تمت الاضافة عن طريق رابط الفيس بوك: https://www.facebook.com/profile.php?id={uidAuthor}\n─────────────────\n[ {time} - {thu} ]` : msg = threadData.customJoin;
      var nameAuthor = await Users.getNameUser(event.author)
      msg = msg
        .replace(/\{iduser}/g, iduser.join(', '))
        .replace(/\{name}/g, nameArray.join(', '))
        .replace(/\{type}/g, (memLength.length > 1) ? 'انت' : 'انت')
