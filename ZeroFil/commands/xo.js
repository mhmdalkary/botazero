...
function move(x, y, data) {
  ...
  } else return "المربع مستخدم بالفعل🗳️❎";
...
module.exports.handleReply = async function({ event, api, handleReply }) {
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    if(checkGameOver(data)) {
      var gayban = ["نوب 😎", "فزت عليك😜", "يالك من مبتدأ😎", "a bit immature 😎", "اوه 🤭 كم انا أسطورة", "easy game 😎"];
      if(checkAIWon(data)) lmao = `You lose! ${gayban[Math.floor(Math.random() * gayban.length)]}`;
      else if(checkPlayerWon(data)) lmao = "أنت الفائز🎉! :<";
      else lmao = "انه تعادل🗿!";
      global.moduleData.tictactoe.delete(threadID);
    }
    var msg = lmao !== "" ? lmao : temp == undefined ? "قم بالرد على الرسالة برقم المربع☑️✍️" : temp;
  ...
  } else return api.sendMessage("هاذا الرقم غير موجود ❎!", threadID, messageID);
}
...
module.exports.run = async function ({ event, api, args }) {
  ...
  let newData;
  if (args.length == 0) return api.sendMessage("من فضلك⚠️ قم بإختيار ✍️ X او O", threadID, messageID);
