module.exports.config = {
  ...
  description: "قم بتشغيل الفيديوهات من يوتيوب",
  usePrefix: true,
  commandCategory: "🄼🄴🄳🄸🄰",
  usages: "يوتيوب [نص]",
  cooldowns: 10,
...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  ytdl.getInfo(handleReply.link[event.body - 1]).then(res => {
    ...
    api.sendMessage(` ⏱️ |جاري تحميل الفيديو\n❍━━━━━━━━━━━━❍\n${body}\n❍━━━━━━━━━━━━❍\nهذا قد يأخذ وقتا `, event.threadID, (err, info) =>
  ...
  try {
    ytdl.getInfo(handleReply.link[event.body - 1]).then(res => {
      ...
      ytdl(handleReply.link[event.body - 1])
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage(' ⚠️ |لايمكن تنزيل الفيديو لأنه أكبر من 25 ميجابايت', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (args.length == 0 || !args) return api.sendMessage(' ⚠️ |قم بإدخال شيء من أجل البحث عن الفيديو', event.threadID, event.messageID);
  ...
  if (urlValid) {
    try {
      ...
      ytdl(args[0])
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400) return api.sendMessage(' ⚠️ |تعذر إرسال الملف لأن حجمه أكبر من 25 ميغابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
    ...
    try {
      ...
      var body = ` ✓ | هناك  ${link.length} تطابق كلمة بحثك :\n\n${msg}\nأرجوك قم بالرد برقم الفيديو اللذي تريد مشاهدته`
      ...
      var body = ` ✓ | هناك ${link.length} نتيحة تطابق كلمة بحثك :\n\n${msg}\nالمرجو إختيار رقم للفيديو أعلاه من أجل مشاهدته `
      return api.sendMessage({ attachment: imgthumnail, body: body }, event.threadID, (error, info) => global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: event.senderID,
